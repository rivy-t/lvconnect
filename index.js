		...
		if (params && params.callback && params.callback.call) {
			...
			return request({
				...
				headers: {
					'User-Agent': agent,
					'Accept': 'application/json',
					'api-secret': crypto.createHash('sha1').update(params.nightscout.API_SECRET).digest(
						'hex',
					),
				},
				body: entries,
...
function toLvapiHost(lvserver) {
	return (lvserver.toUpperCase() === 'CA'
		? 'api-ca.libreview.io'
		: (lvserver.toUpperCase() === 'EU' ? 'api-eu.libreview.io' : (lvserver.toUpperCase() === 'US'
			? 'api-us.libreview.io'
			: (lvserver.indexOf('.') > 1 ? lvserver : 'api.libreview.io'))));
}
...
function engine(params) {
	...
	localTMZ =
		((params.timeOffsetMinutes === '' || params.timeOffsetMinutes === null)
			? new Date().getTimezoneOffset()
			: params.timeOffsetMinutes) * 60;
	...
	function my() {
		return getProCredentials(params, 0) // initial credentials check
			...
			.then((lv) => {
				return convertAll(lv);
			})
			.then((ns) => {
				return uploadToNightscout(params, ns);
			})
			.catch((err) => {
				console.debug(err);
			});
	...
	let params = {
		login: {
			...
			trustedDeviceToken: readENV('LVCONNECT_TRUSTED_DEVICE_TOKEN') ||
				readENV('LVCONNECT_PRO_TRUSTED_DEVICE_TOKEN'),
			...
			return getProCredentials(params, 0) // initial credentials check
				.then(() => {
					return promiseRetry({
						...
						return authorize(params, n).catch(retry);
					});
				})
				.then(() => {
					saveSession();
			...
			restoreSession()
				.then(() => {
					return promiseRetry(
						...
						(retry, n) => {
							console.debug(`lvconnect: attempt to login and fetch data # ${n}`);
							return authorize(params, n).catch(retry);
						},
					);
				})
...
function saveSession() {
	...
	require('fs').writeFile('session.json', JSON.stringify(session), 'utf8', (err) => {
...
function saveData(data) {
	...
	require('fs').writeFile('fetched.json', JSON.stringify(data), 'utf8', (err) => {
		if (err) console.debug(err);
	});
	return data;
}
